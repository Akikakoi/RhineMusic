...
> 本项目基于 [LBEILC/RhineLabUI](https://github.com/LBEILC/RhineLabUI)（MIT License）修改构建，原项目版权声明与许可证见 [LICENSE](LICENSE)。
...
这是对《明日方舟》特别映像「莱茵生命：访问」终端界面的非官方复刻。从白底开场进入五列循环档案阵列，抽取一份档案，等待玻璃与正文解密后阅读内容。

项目以原 PV 的 **5–40 秒**为主要视觉与动效参考，实际开场从 **6.76 秒的白色画面**开始。检索、收藏、正文阅读与声音设置是可操作的扩展功能。
...
检索支持编号、标题、英文名、科室、相关人物与分类筛选。收藏和设置保存在当前浏览器中。音效区分档案的玻璃碰触与系统操作的电子反馈，背景配乐随开场、阵列和详情调整三轨比例。
...
这些脚本检查运动、循环位置、外观、解密轨迹与画质参数。视觉效果仍需在浏览器中实际查看，尤其是快速切换、模型归位与文档揭示。
