...
- 右半区：方图**居中留边**（完整显示，不裁切、不拉伸）。
- 去掉原先的标志水印。
- 左半文字放不下时缩小并加省略号。
- 站点自带曲目（`public/audio`）**不**提取封面。
- 已经导入过的旧曲目没有封面字段就留空，不做回溯重扫。
- 范围外：详情面板、设置里的迷你播放器、其余 39 张阵列卡片、壁纸媒体卡都不显示封面；不做在线抓取与手动选图。
...
- `drawLabel(index)` 改为左右两半：`half = 画布宽 / 2`。
  ...
  - 右半：`drawLabelCover()`。
- `drawLabelCover()`：整张封面按原比例缩放进右半框内居中留边，不裁切、不拉伸；**有封面才画细边框**，没有封面时右半保持纸面留空。
...
- 需求确认：站点曲目不提取、右半区居中留边、去掉水印、左半文字加省略号、旧曲目留空（见 §1）。
